...
public class Search
{
    ...
    public Task<List<SearchResult>> SearchAsync(string searchTerm, bool useAuth = false, CancellationToken cancellationToken = default)
    {
        ...
        string route = _methodPath.SetQueryParam("search", Uri.EscapeDataString(searchTerm));
        return _httpHelper.GetRequestAsync<List<SearchResult>>(route, false, useAuth, cancellationToken: cancellationToken);
    }
