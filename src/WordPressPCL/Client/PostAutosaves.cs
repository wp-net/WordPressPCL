...
public class PostAutosaves
{
    ...
    {
        return _httpHelper.GetRequestAsync<List<PostRevision>>(
            $"posts/{_postId}/{_methodPath}",
            embed,
            isAuthRequired: true,
    ...
    {
        return _httpHelper.GetRequestAsync<PostRevision>(
            $"posts/{_postId}/{_methodPath}/{id}",
            embed,
            isAuthRequired: true,
            cancellationToken: cancellationToken);
    }
    ...
    /// </summary>
    /// <param name="post">The post data to autosave</param>
    /// <param name="cancellationToken">Cancellation token</param>
    ...
    {
        string json = AutosaveSerializer.Serialize(post, _httpHelper.JsonSerializerOptions);
        using StringContent postBody = new(json, Encoding.UTF8, "application/json");
        (PostRevision autosave, _) = await _httpHelper.PostRequestAsync<PostRevision>(
            $"posts/{_postId}/{_methodPath}",
            postBody,
